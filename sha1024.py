...
def sha512(message):
    ...
    for i in range(0, len(message), 128):
        ...
        for j in range(80):
            ...
            ch = (e & f) ^ ((~e) & g)
            temp1 = (hh + S1 + ch + K_512[j] + w[j]) & 0xffffffffffffffff
...
def sha1024(message):
    if isinstance(message, str):
        message = message.encode()
    h1 = sha512(message + b"\x00")
    h2 = sha512(message + b"\x01")
    return h1 + h2